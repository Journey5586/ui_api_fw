...
@modify by :Guozg
@reason:
修正截图等一些问题，同时该文件支持同步模式的多线程(不支持单线程)，想要使用单线程，请使用BeautifulReport_Single.py
"""
...
class ReportTestResult(unittest.TestResult):
    ...
    def __init__(self, suite, stream=sys.stdout):
        ...
        self.txt_path = os.path.join(log_path, txt_name)
        # 必须要初始化case日志txt文件
        self.init_case_log_txt()
        self.__output_msg = Output()
    ...
    def init_case_log_txt(self):
        '''
        初始化case 日志txt文件,若不初始化，是会提示文件找不到。  \n
        主要用于多线程,但也支持单线程  \n
        :return: None
        '''
        with open(self.txt_path, 'w', encoding='utf8') as fp:
            pass

    def read_case_log(self) -> list:
        '''
        读取case日志内容,当test case运行完毕后，调用此方法 \n
        主要用于多线程,但也支持单线程  \n
        :return: list
        '''
        value = ''
        with open(self.txt_path, 'r', encoding='utf8')as fp:
            # self.case_log +=fp.read()
            value = fp.read()
        # return value
        return value.split('\n')

    def clear_case_log(self):
        '''
        清空case日志内容,即每个test case运行完毕后,且读取日志内容后，调用此方法  \n
        主要用于多线程,但也支持单线程  \n
        :return: None
        '''
        with open(self.txt_path, 'w', encoding='utf8') as fp:
    ...
    def del_case_log_file(self):
        '''
        删除case日志txt文件,当所有的case运行结束后，运行此文件  \n
        主要用于多线程,但也支持单线程  \n
        :return: None
        '''
    ...
    def stopTestRun(self, title=None) -> dict:
        ...
        # 当所有的case都运行结束后，必须要删除日志txt文件，否则每次运行case都会新建N个txt文件
        self.del_case_log_file()
        return FIELDS
    ...
    def addSuccess(self, test) -> None:
        ...
        # 下面这种添加的代码会有问题，即：在输出到html中，不会换行输出，所以有的信息都变成了一整行
        # self.case_log.append(self.read_case_log())
        # 读取当前case的日志
        self.case_log += self.read_case_log()
        # 清空case日志文件中的内容。
        self.clear_case_log()
    ...
    def addError(self, test, err):
        ...
        if not getattr(test, "driver", ""):
            ...
            try:
                ...
                # print('\n')
                self.__output_msg.str_msg(HTML_IMG_TEMPLATE.format("Error--出错自动截图", img))
                ...
                # # pass
                self.__output_msg.str_msg(msg)
                self.__output_msg.str_msg('详情信息如下：')
                self.__output_msg.str_msg('traceback.format_exc():')
                self.__output_msg.str_msg(traceback.format_exc())
        ...
        output = self.complete_output()
        logs.append(output)
        # 列表末尾一次性追加另一个序列中的多个值,若传入的是str，则会则str切割成list，如'12345'切割为'1','2','3','4','5'
        # 读取当前case的日志(也就是之前手工输出的print语句)
        logs.extend(self.read_case_log())
        # 清空case日志
        self.clear_case_log()
    ...
    def addFailure(self, test, err):
        ...
        if not getattr(test, "driver", ""):
            ...
            try:
                ...
                # print('\n')
                self.__output_msg.str_msg(HTML_IMG_TEMPLATE.format("Fail--失败自动截图", img))
        ...
        output = self.complete_output()
        logs.append(output)
        # 读取case日志(指的是手工输出的,也就是原来的print语句)
        logs.extend(self.read_case_log())
        # 清空内容
        self.clear_case_log()
        # 将发生错误的日志，添加到报告中
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    #
    #     return _wrap
# help(ReportTestResult)
# help(BeautifulReport)